...
from ..Training import daily_prob, aposteriori
import pandas as pd
...
def score(self,df,robot,value1,query,engine,postGressDataFrame,):
    ...
    score=Training.addMetadata(query,score, engine,postGressDataFrame,column_to_add=robot,value_to_merge=value1)
    if 'event_type' in score.columns:
        score = score.drop(['prob_x','prob_y','event_type'], axis=1)
    else:
        score = score.drop(['prob_x','prob_y'], axis=1)
    lookup_table = Training.getScoreLockUp(score)
